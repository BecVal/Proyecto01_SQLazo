...
class MonthlyLoggerTest {
    ...
    @BeforeEach void cleanUpBefore() throws IOException {
        if (Files.exists(LOG_PATH)) {
            if (Files.isDirectory(LOG_PATH)) Files.delete(LOG_PATH);
            else Files.delete(LOG_PATH);
        }
    }

    @AfterEach void cleanUpAfter() throws IOException {
        if (Files.exists(LOG_PATH)) {
            if (Files.isDirectory(LOG_PATH)) Files.delete(LOG_PATH);
            else Files.delete(LOG_PATH);
        }
    ...
    @Test @DisplayName("update writes one line with the event appended")
    void updateWritesOneLine() throws Exception {
        MonthlyLogger logger = new MonthlyLogger(); String event = "Account opened";
    ...
    @Test @DisplayName("Consecutive updates append lines rather than overwrite")
    void updatesAppend() throws Exception {
        MonthlyLogger logger = new MonthlyLogger(); String e1 = "Deposit: $100"; String e2 = "Withdrawal: $25";
        logger.update(e1); logger.update(e2);
        List<String> lines = Files.readAllLines(LOG_PATH, StandardCharsets.UTF_8);
    ...
    @Test @DisplayName("Swallows IOException when path is a directory")
    void swallowsIOExceptionOnBadPath() throws Exception {
        Files.createDirectory(LOG_PATH); assertTrue(Files.isDirectory(LOG_PATH), "Precondition: path is a directory");
