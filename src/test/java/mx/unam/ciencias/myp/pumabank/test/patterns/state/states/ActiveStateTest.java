...
class ActiveStateTest {
    ...
    static class FakeAccount extends Account {
        private static final Client DUMMY_CLIENT = new Client("John", "ID-1");
        private static final AccountState NOOP_STATE = new AccountState() { @Override public void deposit(double a, Account acc) {} @Override public void withdraw(double a, Account acc) {} @Override public void processMonth(Account acc) {} @Override public void unfreeze(Account acc) {} };
        ...
        double balance = 0.0; final List<String> history = new ArrayList<>(); final List<String> notifications = new ArrayList<>(); AccountState lastStateChange = null; int processMonthCalls = 0; InterestCalculation policy = ZERO_INTEREST;
    ...
    static class AccountHarness {
        ...
        AccountHarness(double initialBalance) { account.balance = initialBalance; }
    ...
    @Nested @DisplayName("Basics: deposit / withdraw") class Basics {
        @Test @DisplayName("deposit adds to balance, records history and notifies") void deposit() {
            AccountHarness h = new AccountHarness(100.0); ActiveState state = new ActiveState(); state.deposit(40.0, h.account);
        ...
        @Test @DisplayName("withdraw with sufficient funds subtracts and records") void withdrawSufficient() {
            AccountHarness h = new AccountHarness(200.0); ActiveState state = new ActiveState(); state.withdraw(80.0, h.account);
        ...
        @Test @DisplayName("withdraw with insufficient funds triggers OverdrawnState") void withdrawInsufficient() {
            AccountHarness h = new AccountHarness(50.0); ActiveState state = new ActiveState(); state.withdraw(120.0, h.account);
            ...
            assertNotNull(h.account.lastStateChange); assertEquals("OverdrawnState", h.account.lastStateChange.getClass().getSimpleName());
        }
    }

    @Nested @DisplayName("processMonth with MonthlyInterest") class MonthlyInterestTests {
        @Test @DisplayName("Applies monthly interest when balance >= minimum") void monthlyApplies() {
            AccountHarness h = new AccountHarness(1000.0); h.account.policy = new MonthlyInterest(0.01, 500.0); ActiveState state = new ActiveState(); state.processMonth(h.account);
        ...
        @Test @DisplayName("Records 'no interest' when below minimum or zero") void monthlyNoInterest() {
            AccountHarness h = new AccountHarness(400.0); h.account.policy = new MonthlyInterest(0.02, 500.0); ActiveState state = new ActiveState(); state.processMonth(h.account);
    ...
    @Nested @DisplayName("processMonth with PremiumInterest") class PremiumInterestTests {
        @Test @DisplayName("Applies base + tiered bonus on high balances") void premiumTiers() {
            AccountHarness h = new AccountHarness(20000.0); h.account.policy = new PremiumInterest(0.005, 10000, 20000, 0.003, 0.006); ActiveState state = new ActiveState(); state.processMonth(h.account);
        ...
        @Test @DisplayName("No interest when balance <= 0") void premiumZeroOrNegative() {
            AccountHarness h = new AccountHarness(0.0); h.account.policy = new PremiumInterest(0.01, 1000, 5000, 0.005, 0.01); ActiveState state = new ActiveState(); state.processMonth(h.account);
            assertAll(() -> assertEquals(0.0, h.balance(), 1e-9),() -> assertTrue(h.history().stream().anyMatch(s -> s.equals("Monthly processing: no interest applied."))),() -> assertTrue(h.notifications().stream().anyMatch(s -> s.equals("MONTHLY_PROCESSING: No interest applied to active account"))),() -> assertTrue(h.notifications().stream().anyMatch(s -> s.equals("MONTHLY_SUMMARY: Active account processed"))));
        }
    }

    @Nested @DisplayName("processMonth with AnnualInterest") class AnnualInterestTests {
        @Test @DisplayName("Months 1..11: returns 0.0 interest") void annualNonPayoutMonths() {
            AccountHarness h = new AccountHarness(3000.0); AnnualInterest calc = new AnnualInterest(0.12, 2000.0); calc.setCurrentMonth(5); calc.recordMonthBalance(2500.0); calc.recordMonthBalance(3000.0); h.account.policy = calc; ActiveState state = new ActiveState(); state.processMonth(h.account);
            assertAll(() -> assertEquals(3000.0, h.balance(), 1e-9),() -> assertTrue(h.history().stream().anyMatch(s -> s.equals("Monthly processing: no interest applied."))),() -> assertTrue(h.notifications().stream().anyMatch(s -> s.equals("MONTHLY_PROCESSING: No interest applied to active account"))),() -> assertTrue(h.notifications().stream().anyMatch(s -> s.equals("MONTHLY_SUMMARY: Active account processed"))));
        }
        @Test @DisplayName("Month 12 with avg above threshold pays annual interest on current balance") void annualPayoutMonth() {
            AccountHarness h = new AccountHarness(5000.0); AnnualInterest calc = new AnnualInterest(0.12, 2000.0); calc.recordMonthBalance(2500.0); calc.recordMonthBalance(3000.0); calc.recordMonthBalance(2000.0); calc.setCurrentMonth(12); h.account.policy = calc; ActiveState state = new ActiveState(); state.processMonth(h.account);
    ...
    @Nested @DisplayName("Negative balance path") class NegativePath {
        @Test @DisplayName("If balance is negative, switch to OverdrawnState and delegate .processMonth()") void negativeBalanceDelegatesToOverdrawn() {
            AccountHarness h = new AccountHarness(-1.0); ActiveState state = new ActiveState(); state.processMonth(h.account);
            assertNotNull(h.account.lastStateChange); assertEquals("OverdrawnState", h.account.lastStateChange.getClass().getSimpleName());
    ...
    @Nested @DisplayName("unfreeze") class Unfreeze {
        @Test @DisplayName("Already active: only notifies and records") void alreadyActive() {
            AccountHarness h = new AccountHarness(100.0); ActiveState state = new ActiveState(); state.unfreeze(h.account);
