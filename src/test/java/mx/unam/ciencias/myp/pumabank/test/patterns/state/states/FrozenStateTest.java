...
class FrozenStateTest {
    ...
    class DepositFrozen {
        ...
        void depositDenied() {
            ...
            assertAll(
                () -> assertTrue(acc.history.stream().anyMatch(s -> s.equals("Deposit denied: account is frozen. Attempted: $200.0"))),
                () -> assertTrue(acc.notifications.stream().anyMatch(s -> s.equals("The operation of deposit on frozen account was blocked."))),
                () -> assertEquals(150.0, acc.getBalance(), 1e-9),
                () -> assertEquals(0, acc.changeStateCalls),
                () -> assertNull(acc.lastStateChange)
            );
        }
    }
