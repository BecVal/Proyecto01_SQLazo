...
class AccountTest {
  ...
  static class RecordingState implements AccountState {
    boolean depositCalled; boolean withdrawCalled; boolean processCalled; boolean unfreezeCalled; double lastAmount;
    @Override public void deposit(double amount, Account account) { depositCalled = true; lastAmount = amount; account.addHistory("state:deposit " + amount); account.setBalance(account.getBalance() + amount); }
    @Override public void withdraw(double amount, Account account) { withdrawCalled = true; lastAmount = amount; account.addHistory("state:withdraw " + amount); account.setBalance(account.getBalance() - amount); }
    @Override public void processMonth(Account account) { processCalled = true; account.addHistory("state:processMonth"); }
    @Override public void unfreeze(Account account) { unfreezeCalled = true; account.addHistory("state:unfreeze"); }
  }

  static class RecordingObserver implements Observer { final List<String> messages = new ArrayList<>(); @Override public void update(String message) { messages.add(message); } }
  static class ThrowingObserver implements Observer { @Override public void update(String message) { throw new RuntimeException(); } }
  static class FixedRateInterest implements InterestCalculation { private final double rate; FixedRateInterest(double rate) { this.rate = rate; } @Override public double calculate(double balance) { return balance * rate; } }
  ...
  void constructorRejectsNulls() {
    RecordingState st = new RecordingState(); InterestCalculation ic = new FixedRateInterest(0.01); Client c = anyClient(); PumaBankFacade f = facade();
  ...
  void depositWithPositiveAmountDelegatesToStateAndUpdatesBalance() {
    RecordingState st = new RecordingState(); Account acc = new Account(anyClient(), 100.0, st, new FixedRateInterest(0.0), facade());
  ...
  void processMonthDelegatesToState() {
    RecordingState st = new RecordingState(); Account acc = new Account(anyClient(), 50.0, st, new FixedRateInterest(0.0), facade());
    acc.processMonth();
  ...
  void changeStateAffectsSubsequentOperations() {
    RecordingState st1 = new RecordingState(); RecordingState st2 = new RecordingState(); Account acc = new Account(anyClient(), 10.0, st1, new FixedRateInterest(0.0), facade());
    acc.changeState(st2); acc.deposit(5.0, "x");
  ...
  void setInterestPolicyUpdatesAndAddsHistory() {
    RecordingState st = new RecordingState(); Account acc = new Account(anyClient(), 100.0, st, new FixedRateInterest(0.01), facade());
    InterestCalculation newPolicy = new FixedRateInterest(0.02); acc.setInterestPolicy(newPolicy);
  ...
  void notifyCallsAllObserversAndSwallowsExceptions() {
    Account acc = new Account(anyClient(), 0.0, new RecordingState(), new FixedRateInterest(0.0), facade()); RecordingObserver a = new RecordingObserver(); RecordingObserver b = new RecordingObserver();
    acc.addObserver(a); acc.addObserver(new ThrowingObserver()); acc.addObserver(b);
    acc.notify("hello");
  ...
  void removeObserverStopsReceivingNotifications() {
    Account acc = new Account(anyClient(), 0.0, new RecordingState(), new FixedRateInterest(0.0), facade()); RecordingObserver a = new RecordingObserver();
    acc.addObserver(a); acc.removeObserver(a); acc.notify("ignored");
    assertTrue(a.messages.isEmpty());
  }

  @Test
  void setBalanceAndGetBalanceWork() {
    Account acc = new Account(anyClient(), 0.0, new RecordingState(), new FixedRateInterest(0.0), facade()); acc.setBalance(123.45);
