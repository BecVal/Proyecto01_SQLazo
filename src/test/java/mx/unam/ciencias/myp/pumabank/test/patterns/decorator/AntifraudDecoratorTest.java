...
class AntiFraudDecoratorTest {
    ...
    static class RecordingAccount implements IAccount {
        double lastDepositAmount; String lastDepositPin; int depositCalls; double lastWithdrawAmount; String lastWithdrawPin; int withdrawCalls; int processMonthCalls; double balanceToReturn = 0.0; String lastCheckBalancePin; final List<String> history = new ArrayList<>(); final List<String> notifications = new ArrayList<>();
    ...
    @Nested @DisplayName("Deposit behavior") class DepositBehavior {
        @Test @DisplayName("Delegates deposit with amount and pin") void delegatesDeposit() {
            RecordingAccount base = new RecordingAccount(); AntiFraudDecorator deco = new AntiFraudDecorator(base); deco.deposit(500.0, "1234");
        ...
        @Test @DisplayName("No hooks when amount <= 10000") void noHooksAtOrBelowThreshold() {
            RecordingAccount base = new RecordingAccount(); AntiFraudDecorator deco = new AntiFraudDecorator(base); deco.deposit(10000.0, "pin");
            assertAll(() -> assertEquals(1, base.depositCalls),() -> assertTrue(base.history.isEmpty()),() -> assertTrue(base.notifications.isEmpty()));
        }
        @Test @DisplayName("Triggers hooks when amount > 10000") void triggersHooksAboveThreshold() {
            RecordingAccount base = new RecordingAccount(); CaptureDecorator mid = new CaptureDecorator(base); AntiFraudDecorator deco = new AntiFraudDecorator(mid); double amount = 15000.0; deco.deposit(amount, "9999");
    ...
    @Nested @DisplayName("Withdraw behavior") class WithdrawBehavior {
        @Test @DisplayName("Delegates withdraw with amount and pin") void delegatesWithdraw() {
            RecordingAccount base = new RecordingAccount(); AntiFraudDecorator deco = new AntiFraudDecorator(base); deco.withdraw(200.0, "0000");
        ...
        @Test @DisplayName("No hooks when amount <= 10000") void noHooksAtOrBelowThreshold() {
            RecordingAccount base = new RecordingAccount(); AntiFraudDecorator deco = new AntiFraudDecorator(base); deco.withdraw(10000.0, "pin");
            assertAll(() -> assertEquals(1, base.withdrawCalls),() -> assertTrue(base.history.isEmpty()),() -> assertTrue(base.notifications.isEmpty()));
        }
        @Test @DisplayName("Triggers hooks when amount > 10000") void triggersHooksAboveThreshold() {
            RecordingAccount base = new RecordingAccount(); CaptureDecorator mid = new CaptureDecorator(base); AntiFraudDecorator deco = new AntiFraudDecorator(mid); double amount = 20000.0; deco.withdraw(amount, "1111");
    ...
    @Nested @DisplayName("Monthly processing") class MonthlyProcessing {
        @Test @DisplayName("Applies anti-fraud fee, records history/notification, and delegates processMonth") void appliesMonthlyFeeAndDelegates() {
            RecordingAccount base = new RecordingAccount(); CaptureDecorator mid = new CaptureDecorator(base); AntiFraudDecorator deco = new AntiFraudDecorator(mid); deco.processMonth();
